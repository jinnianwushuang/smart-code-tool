...
import { genarate_event_listener, genarate_event_pipeline, genarate_singleton } from './config.js'
 ...
 *
 */
export const assemble_lifecycle_centralized = (all_params) => {
  ...
  onBeforeMount(() => {
    ...
    singleton_fn()
    // 执行传入的生命周期回调函数
    lifecycle?.lifecycle_onBeforeMount?.()
    // 事件管道的函数 需要在生命周期开始前就生成好，供组件内其他函数调用
    event_pipeline_fn()
  })
  onMounted(() => {
    // 执行传入的生命周期回调函数
    lifecycle?.lifecycle_onMounted?.()
  })
  onBeforeUnmount(() => {
    // 执行传入的生命周期回调函数
    lifecycle?.lifecycle_onBeforeUnmount?.()
  })
  onUnmounted(() => {
    // 执行传入的生命周期回调函数
    lifecycle?.lifecycle_onUnmounted?.()
    // 组件生命周期结束时，执行一次单例函数，进行清理工作
    singleton_fn()
  })
  onActivated(() => {
    // 执行传入的生命周期回调函数
    lifecycle?.lifecycle_onActivated?.()
  })
  onDeactivated(() => {
    // 执行传入的生命周期回调函数
    lifecycle?.lifecycle_onDeactivated?.()
  })
