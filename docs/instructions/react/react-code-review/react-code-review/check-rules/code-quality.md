...
- **严重级别**：🟡 Warning
- **检查方式**：统计每个 `.tsx` / `.jsx` / `.ts` / `.js` 文件的总行数
- **处理建议**：超过阈值则建议拆分为子组件或抽取自定义 Hook / 工具函数

### 2. 单函数体最大行数

- **默认阈值**：50 行（可通过 config.md 的 `max_function_lines` 配置）
- **严重级别**：🟡 Warning
- **检查方式**：检查每个函数/组件定义体的行数
- **处理建议**：超过阈值则建议拆分为多个子函数或子组件
...
- **处理建议**：不存在则建议创建 ESLint 配置（React 19 推荐 Flat Config）

### 5. ESLint 是否包含 React 推荐规则

- **严重级别**：🔵 Info
- **检查方式**：检查 ESLint 配置中是否包含 `eslint-plugin-react` 与 `eslint-plugin-react-hooks`
- **处理建议**：未包含则建议安装并配置
...
- **命名规则**（可通过 config.md 配置）：
  - 文件名：`kebab-case`（如 `user-list.tsx`）或组件文件 `PascalCase`（如 `UserList.tsx`），全项目须统一
