...
第 3 步：读取架构参照（按需）
  → architecture/component-reference.md（组件模式）
  → architecture/hooks-reference.md（自定义 Hooks）
  → architecture/state-reference.md（状态管理）
  → architecture/actions-reference.md（Actions 模式）
  → architecture/rsc-reference.md（Server/Client 边界）
...
4. CSS 复用 — UI 框架组件 > 插件 > 全局 CSS > CSS Modules
5. 验证边界 — 默认只做静态分析，不主动启动项目
6. Compiler 兼容 — 禁止手写 useMemo/useCallback/forwardRef
