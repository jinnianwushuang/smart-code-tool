...
第 3 步：读取架构参照（按需）
  → architecture/assembler-reference.md（装配器模式）
  → architecture/state-reference.md（状态管理）
  → architecture/lifecycle-reference.md（生命周期与副作用）
  → architecture/event-pipeline-reference.md（事件管道）
  → architecture/component-reference.md（组件系统）
  → architecture/api-request-reference.md（API 请求，仅单例模板）
