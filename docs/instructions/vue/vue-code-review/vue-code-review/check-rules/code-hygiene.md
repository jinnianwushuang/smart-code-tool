...
- **严重级别**：🟡 Warning
- **检查方式**：扫描所有 `.js` / `.ts` / `.vue` 文件中的 `console.log`、`console.debug`、`debugger` 语句
- **排除范围**：
...
- **严重级别**：🟡 Warning
- **检查方式**：检查是否存在相似逻辑在多处出现但未抽取为公共函数/组件
- **判断标准**：
  - 连续 10 行以上高度相似的代码
  - 相同业务逻辑在不同组件中重复实现
- **处理建议**：
  - 重复逻辑抽取为 composable（`use-*.js`）
  - 重复 UI 模式抽取为公共组件
  - 重复配置抽取为常量文件
