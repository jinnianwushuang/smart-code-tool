...
> 每个框架的提示词代码块首行均要求 AI 助手先读取对应的**约束文件**（`constraints.md`）作为基础上下文，包含版本基线、编码约定、架构规范等共享约束，使提示词本身只需关注场景特定需求。

<a href="/archive/prompt-constraints.zip" class="download-btn" download>
  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
  下载约束文件合集
</a>

> 压缩包包含 Vue / React / Flutter 三个框架的 `constraints.md`，拷贝到项目后供提示词引用。
...
> 详见 [Vue 提示词集](./vue/prompts)（约束上下文：[Vue 约束](./vue/constraints)）
...
> 详见 [React 提示词集](./react/prompts)（基于 React 19，约束上下文：[React 约束](./react/constraints)）
...
> 详见 [Flutter 提示词集](./flutter/prompts)（基于 Flutter 3.24+ + GetX 4.6+，约束上下文：[Flutter 约束](./flutter/constraints)）
...
3. 复制代码块中的内容（首行会自动引导 AI 读取框架约束文件）
4. 替换 `[方括号]` 为你的实际参数
5. 粘贴到 AI 聊天框发送

> 约束文件位于各框架目录下：[Vue](./vue/constraints)、[React](./react/constraints)、[Flutter](./flutter/constraints)。AI 助手读取后自动获得版本基线、编码约定等共享上下文，无需每条提示词重复声明。
