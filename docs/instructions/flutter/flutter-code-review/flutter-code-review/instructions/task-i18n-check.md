...
参照 `check-rules/i18n-check.md` 第 1 项，扫描 Widget 中的中文文本（如 `Text('加载中')`）是否未使用 `.tr` / `AppLocalizations`。

### 步骤 3：检查 Dart 逻辑硬编码

参照 `check-rules/i18n-check.md` 第 2 项，扫描 Dart 代码中的用户可见中文字符串（如 SnackBar、Dialog 提示）。

### 步骤 4：多语种键值对比

如 config.md 中 `i18n_locale_files` 已填写：

1. 读取所有语种文件（arb / GetX translations Map）
2. 递归对比各语种的 key 结构
3. 输出缺失键列表
