...
> 面向所有 Flutter 3 + GetX 项目的通用代码质量检查指令集，不绑定特定业务架构。

## 概述

本指令集为 AI 助手提供 **10 大检查维度、57 个检查项**，覆盖代码规范、Flutter Widget、性能、内存管理、并发处理、国际化、安全、错误处理、组件设计和代码卫生。

**版本基线**：Flutter 3.24+ / Dart 3.5+ / GetX 4.6+，默认启用 Material 3、null safety、Impeller 渲染引擎。
...
将 `flutter-code-review/` 目录下的 **`flutter-code-review/`** 子目录拷贝到你的项目中：
...
编辑 `config.md`，至少填写 `state_management`。参照 [配置指南](./config-guide.md) 了解详情。
...
```
请读取 flutter-code-review/entry/context-entry.md 并按顺序建立上下文
```
...
```
任务类型：深度代码审查
检查目录：lib/modules/user/    ← 可选，不填则全项目
报告地址：默认
