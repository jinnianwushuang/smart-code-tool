...
const activeTab = ref('progress') // 'progress' | 'doubt'
...
// ==================== 计算属性 ====================
const totalCount = computed(() => progressRecords.value.length + doubtRecords.value.length)
      ...
      <div v-if="isOpen" class="qt-overlay" @click.self="closePanel">
        ...
        <div class="qt-panel">
          ...
          <div class="qt-tabs">
            ...
            >
              📖 记忆进度 ({{ progressRecords.length }}/15)
            </button>
            <button :class="['qt-tab', { active: activeTab === 'doubt' }]" @click="switchToDoubt">
              ❓ 记忆疑惑 ({{ unresolvedDoubtCount }}/{{ doubtRecords.length }})
            </button>
          </div>
