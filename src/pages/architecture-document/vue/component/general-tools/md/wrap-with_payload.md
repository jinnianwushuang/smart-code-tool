 ...
 */
const create_context_dispatcher = (payload, fn_obj) => {
...
// 3. 执行封装
const actions = create_context_dispatcher(my_payload, user_actions);
...
你是否需要在这个封装中加入**中间件（Middleware）**逻辑，例如在每个函数执行前后自动开启和关闭 **Loading** 状态？